...
template<typename T>
T JsonParser::get(const picojson::object& jsonObject, const std::string& key, const T& valueToReturnOnFailed) {
    if(jsonObject.find(key.c_str()) == jsonObject.end()){
//        std::cout << "JsonParser::Get not found key:" << key << std::endl;
        return valueToReturnOnFailed;
    }
    if(!jsonObject.at(key.c_str()).is<T>()){
        std::cout << "JsonParser::Get type is not matched key:" << key << std::endl;
        return valueToReturnOnFailed;
    }
    return jsonObject.at(key.c_str()).get<T>();
}

template double JsonParser::get<double>(const picojson::object& jsonObject, const std::string& key, const double& valueToReturnOnFailed);
